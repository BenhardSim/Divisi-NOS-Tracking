    ...
    public function index(){
        if (Gate::allows('staff')) {
            return view('portal.sign', [
                "title" => "Sign Document",
                "documents" => tracked_document::where("level_approval", 0)->where("id_pengirim", auth()->user()->id)->orderBy('status', 'ASC')->get(),
        ...
        if (Gate::allows('supervisor')) {
            return view('portal.sign', [
                "title" => "Sign Document",
                "documents" => tracked_document::where("level_approval", 1)->where("id_level_dua", auth()->user()->id)->orderBy('status', 'ASC')->get(),
        ...
        if (Gate::allows('manager')) {
            return view('portal.sign', [
                "title" => "Sign Document",
                "documents" => tracked_document::where("level_approval", 2)->where("id_level_tiga", auth()->user()->id)->orderBy('status', 'ASC')->get(),
        ...
        if (Gate::allows('gm')) {
            return view('portal.sign', [
                "title" => "Sign Document",
                "documents" => tracked_document::where("level_approval", 3)->where("id_level_empat", auth()->user()->id)->where("status", "Pending")->orderBy('status', 'ASC')->get(),
